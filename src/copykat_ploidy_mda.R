#!/bin/Rscript
#Usage: Rscript copykat_ploidy_mda.R 
#Mixture: 4K diploid + 1K baseline + 1K mda per condition - total 3 conditions i.e 1,5,10 DPT (total 8K)
#Description: R script to run copyKAT on MDA cells + spikein 
...
for (prefix in treatment_prefixes) {
  ...
  #untreated is baseline, c1 is cisplatin day 1, c5 is cisplatin day 5, c10 is cisplatin day 10, same for docetaxel
  for (tp in c("untreated", paste0(prefix, c("1", "5", "10")))) {
    cells_tp <- WhichCells(mda, expression = Group ==  tp) #Group colname denotes condition
  ...
  sc_combine <- merge(
    mda_subset_list[[1]],
    y = c(mda_subset_list[[2]], mda_subset_list[[3]],mda_subset_list[[4]] , spikein),
  ...
  # Save results
  cat("Saving results\n")
  saveRDS(object = list(copykat.res, sc_combine@meta.data), file = paste0(proj_wd, "/R/copykatres_mda_", prefix, ".rds"))
  saveRDS(object = sc_combine, file = paste0(proj_wd, "/R/scrna_ds_mda_", prefix, ".rds"))
